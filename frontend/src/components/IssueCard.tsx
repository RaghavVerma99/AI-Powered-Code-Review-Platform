...
export default function IssueCard({ issue, index }: IssueCardProps) {
  ...
  return (
    <div
      className={`glass rounded-xl border ${cfg.border} ${cfg.bg} p-4 animate-fade-in`}
      ...
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            ...
            <span className="rounded-md bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 text-[11px] font-mono text-surface-400">
              {issue.category}
            </span>
            {issue.line > 0 && (
              <span className="text-[11px] font-mono text-surface-500">
                L{issue.line}{issue.column ? `:${issue.column}` : ""}
              </span>
            )}
          </div>
