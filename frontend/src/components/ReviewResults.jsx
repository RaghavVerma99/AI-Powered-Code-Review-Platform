...
export default function ReviewResults({ score, summary, issues, onBack }) {
  ...
  return (
    <div className="space-y-6">
      <button onClick={onBack} className="btn-ghost text-sm group">
        <ArrowLeft size={16} className="transition-transform group-hover:-translate-x-0.5" /> Back to Editor
      </button>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
          <div className="flex items-center gap-4">
            <div className={`relative flex h-20 w-20 items-center justify-center rounded-2xl ${meta.bg} border ${meta.border} ring-2 ${meta.ring}`}>
              ...
              <span className={`absolute -bottom-1 -right-1 text-xs font-bold ${meta.color} bg-surface-950 rounded-full px-1.5 py-0.5 border ${meta.border}`}>{score}</span>
            </div>
            <div>
              ...
              <div className="flex gap-3 mt-2">
                {counts.error > 0 && <span className="flex items-center gap-1 text-xs text-red-400"><AlertOctagon size={12} /> {counts.error}</span>}
                {counts.warning > 0 && <span className="flex items-center gap-1 text-xs text-amber-400"><AlertTriangle size={12} /> {counts.warning}</span>}
                {counts.info > 0 && <span className="flex items-center gap-1 text-xs text-blue-400"><Info size={12} /> {counts.info}</span>}
              </div>
            </div>
          </div>
          {summary && (
      ...
      {issues.length === 0 ? (
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-surface-200">Found {issues.length} Issue{issues.length !== 1 ? "s" : ""}</h3>
            <span className="text-xs text-surface-500">Sorted by severity</span>
          </div>
